...
<template>
  ...
  <div class="toast toast-bottom toast-end z-[9999]">
    ...
    >
      <div
        v-if="showTestNotice"
        class="alert alert-info shadow-lg max-w-sm border border-info bg-info text-info-content"
        role="status"
      >
        <Info class="w-4 h-4" />
        ...
        <div class="max-h-[300px] overflow-auto pr-1 space-y-3">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <button class="btn btn-xs " @click="emit('remove', item)">
                <Minus class="w-4 h-4" />
              </button>

              <button class="btn btn-xs btn-round btn-primary" @click="emit('add', item)">
                <Plus class="w-4 h-4" />
              </button>
            </div>
          </div>
